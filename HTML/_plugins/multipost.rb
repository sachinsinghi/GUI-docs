...
module Jekyll
  module PermalinkBuilder
    ...
    def get_adjusted_permalink(resource, layout)
      ...
      url = resource.url #/:layout/:title/
      ext = File.extname(url)

      if url.include?(':layout')
        # puts File.dirname( resource.path ) + '/' + File.basename( resource.path, ".md" ) + url.gsub(/:layout/, layout_path)
        return File.dirname( resource.path ) + '/' + File.basename( resource.path, ".md" ) + url.gsub(/:layout/, layout_path)
      end
